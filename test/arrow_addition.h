...
void test_arrow_addition() {
  auto fnCheckArrow = [](const Arrow::List &morphs_, const Arrow &morph_) {
    auto it = std::find_if(morphs_.begin(), morphs_.end(),
                           [&](const Arrow::List::value_type &element_) {
                             return element_.Name() == morph_.Name() &&
                                    element_.Source() == morph_.Source() &&
                                    element_.Target() == morph_.Target();
                           });
...

  {
    ...
    Node a("a", Node::EType::eObject), b("b", Node::EType::eObject),
        c("c", Node::EType::eObject), d("d", Node::EType::eObject);
    ...
    assert(fnCheckArrow(cat.QueryArrows(Arrow("*", "*").AsQuery()),
                        Arrow(a, b, "f0")));
  ...
  // Checking identity arrow
  {
    ...
    auto arrows_btw_objects =
        arrows.front().QueryArrows(Arrow("*", "*").AsQuery());
